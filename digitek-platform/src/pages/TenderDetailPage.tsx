...
import { shouldSkipStage } from '../modules/tenders/data/gateways'
import type { AmountBand, TenderStage } from '../modules/tenders/types'
...
type Tab = 'overview' | 'documents' | 'committees' | 'vendors' | 'milestones' | 'audit'
...
export function TenderDetailPage() {
  ...
  const { tender, budget, documents, proposals, contracts, milestones, protocols, personas, auditLog, loading, error } = detail

  const stageOrder = useMemo(() => {
    if (!tender) return []
    return STAGES.map(s => {
      const skip = shouldSkipStage(s.code, { amount: tender.estimated_amount, selection: tender.selection_type })
      const currentIdx = STAGES.findIndex(x => x.code === tender.current_stage)
      const thisIdx = STAGES.findIndex(x => x.code === s.code)
      let state: 'done' | 'current' | 'pending' | 'skipped' = 'pending'
      if (skip.skip) state = 'skipped'
      else if (thisIdx < currentIdx) state = 'done'
      else if (s.code === tender.current_stage) state = 'current'
      return { stage: s, state, skipReason: skip.reason }
    })
  }, [tender])
    ...
    <div className={styles.page}>
      ...
      <div className={styles.kpiRow}>
        <div className={styles.kpiCard}>
          ...
          <div className={styles.kpiValue}>{currentStageDef ? `${currentStageDef.stageNumber}. ${currentStageDef.label.slice(0, 28)}` : tender.current_stage}</div>
        </div>
        <div className={styles.kpiCard}>
          <div className={styles.kpiLabel}>סכום משוער</div>
          ...
          <div className={styles.overviewGrid}>
            <div>
              ...
              <div className={styles.section}>
                ...
                {budget ? (
                  <>
                    <div className={styles.fact}><span className={styles.factLabel}>סכום מאושר</span><span className={styles.factValue}>{formatAmount(budget.amount)}</span></div>
                ...
                ) : <div className={styles.emptyTab}>אין תקציב מאושר עדיין</div>}
              </div>

              <div className={styles.section}>
                <div className={styles.sectionTitle}>צוות ההליך ({personas.length})</div>
                {personas.length === 0 ? (
                  <div className={styles.emptyTab}>אין משתתפים נוספים</div>
                ) : (
            ...
            <div>
              <div className={styles.sectionTitle}>התקדמות שלבים</div>
              <div className={styles.stageProgress}>
                <div className={styles.stageList}>
                  {stageOrder.map(({ stage, state, skipReason }) => (
                    <div key={stage.code} className={styles.stageRow} title={skipReason}>
                      <div className={`${styles.stageDot} ${styles[state] ?? ''}`} />
                      <span className={`${styles.stageName} ${state === 'current' ? styles.current : ''} ${state === 'skipped' ? styles.skipped : ''}`}>

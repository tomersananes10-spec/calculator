...
| 26.06 | **feat(knowledge-hub): מודול חדש "מרכז ידע" (OneStopShop) — מסכי חיפוש דמוי-Google + מסעות מודרכים**. דף הבית של LIBA הוחלף מ-Dashboard ל-`KnowledgeHubPage`: כותרת ענק "LIBA" + תיבת חיפוש pill ממורכזת + chips של דוגמאות. המשתמש מקליד חזון/בעיה בטקסט חופשי, Gemini Flash 2.5 (דרך `/api/ai-advisor` הקיים, structured output עם JSON schema) מחזיר 1-10 שלבים מהקטלוג של 6 המודולים (brief/takam/aiml/tenders/roved5/suppliers). כל שלב מציג קישור עם **pre-fill עמוק**: לחיצה על "פתח" מנווטת למודול עם `journey_step_id` + פרמטרים בURL — `BriefGenerator` יוצר טיוטה חדשה עם cluster_id + title + background, `Calculator` (takam/aiml) ממלא name/ministry, `TenderWizardPage` ממלא brief_id/calculation_id, `Roved5`/`Suppliers` מסננים לפי category/cluster. **קישור דו-כיווני**: 3 migrations חדשות (037-039) — `journeys` + `journey_steps` עם RLS פר משתמש, FK חדשים `journey_step_id` ב-`briefs`/`calculations`/`tenders`, ו-DB trigger שמסמן את ה-step כ-done על INSERT/UPDATE (מעקב את ה-entity שנוצר). AIML המקומי + Roved5/Suppliers מסמנים done דרך client. **קבצים חדשים**: [src/modules/knowledge-hub/](digitek-platform/src/modules/knowledge-hub/) — `KnowledgeHubPage`, `JourneyResultView`, `hooks/useJourney`+`useJourneys`, `lib/advisor`+`promptBuilder`+`moduleCatalog`+`stepActions`, `types.ts`. **מחיקת Dashboard.tsx + Dashboard.module.css** ("בית" בסיידבר → "מרכז ידע" עם 💡). פאנל "המסעות שלי" מובנה במסך החיפוש (5 אחרונים, ע"י לחיצה פותח את המסע השמור). מסך החיפוש עצמו (אמפטי-סטייט): פונט Heebo 84px לכותרת, תיבת pill 640px, 4 צ'יפס דוגמאות. מצב תוצאות: header מינימלי עם "LIBA" + כפתור "חיפוש חדש", אז `JourneyResultView` עם 6 כרטיסי שלבים, progress bar, ו-CTA primary "פתח עם הקשר מועבר" פר שלב. `npx tsc --noEmit` עבר נקי. אימות E2E (DB + API + UI) בוצע ב-Vercel preview. |
...
| 06.08 | **fix(layout): position:sticky היה שבור בכל האפליקציה — הסרת overflow-y:auto מ-AppLayout .main**. המשתמש דיווח (בצדק, פעמיים) שכרטיס הסיכום בשלב 3 לא צף. אבחון עם Playwright מקומי: `.main` של AppLayout הוגדר `overflow-y: auto`, אבל `.layout` הוא `min-height: 100vh` (גדל חופשי) — לכן `.main` מעולם לא גלל בפועל (הגלילה על ה-window). הורה עם overflow≠visible הופך לרפרנס של sticky → כל sticky בתוך האפליקציה מת בשקט מאז ומעולם: כרטיס שלב 3 החדש, הכרטיס הסגול של שלב 4 (TAKAM + AIML), StageMap במכרזים. התיקון: הסרת השורה (no-op לגלילה) + comment הסבר. אומת ב-Playwright: אחרי גלילה הכרטיס ננעל על top:20px בדיוק. לקח: הבדיקה הראשונה שלי פוענחה לא נכון (ראיתי שארית כרטיס סטטי וחשבתי שהוא דבוק) — המשתמש התעקש ובצדק. |
| 06.08 | **feat(aiml-calc): סה"כ חי בשלב 3 — כרטיס סיכום צמוד-צד (מוקאפ C נבחר)**. שלב 3 עבר לפריסת 2 עמודות (`s.mainWide` + `s.twoCol` כמו שלב 4): שמאל כרטיסי התוצרים, ימין הכרטיס הסגול (`s.summaryCard` ממוחזר) sticky שמתעדכן חי בכל שינוי כמות — סה"כ עם אנימציית pulse (`key={total}` remount), תוצרים/יחידות/מע"מ/סה"כ כולל. **תקציב יעד**: שדה חדש `budgetTarget` ב-state (נשמר ב-localStorage + בחישובים שמורים), קלט באלפי ₪, progress bar ירוק/כתום (85%+)/אדום עם הודעת "✓ נותרו X" או "⚠ חריגה של X". החיווי מופיע גם בכרטיס שלב 4. הבר הכהה הישן (`summaryBar`) הוסר + CSS יתום נמחק. מובייל: הכרטיס קופץ מעל הרשימה (order:-1) sticky-top במצב קומפקטי (breakdown מוסתר). `npx tsc --noEmit` עבר נקי. |
...
| 31.08 | **ops(eligibility): הגירה מ-DB עצמאי מושהה ל-coe-hub סכמת `eligibility` (לפני הקפאה לצמיתות)**. התקבל מייל מ-Supabase שפרויקט `eligibility-engine` (`ksosadyupiflqselvybv`) יוקפא ב-5 ימים אחרי 85 יום השהיה. אי אפשר לשחזר אותו כפרויקט נפרד — מכסת 2 פרויקטים חינמיים פעילים תפוסה ע"י digitek-dev (LIBA) + coe-hub (COE). לכן הוגר לסכמה `eligibility` ב-coe-hub לפי תבנית המחיצות (כמו recruit/trips). המבנה שוחזר **מהקוד** (מקור האמת — ה-frontend מגדיר טבלאות/עמודות) בלי לשחזר את ה-DB המושהה: migration עם 4 טבלאות (`candidates`, `eligibility_checks`, `decisions`, `audit_log`) + 2 triggers (check_created/decision_made→audit) + RLS פרמיסיבי scoped ל-anon+authenticated (כלי פנימי BYPASS) + grants + bucket `cv-files`. חשיפת הסכמה ל-Data API דרך `pgrst.db_schemas` על authenticator (`public, graphql_public, recruit, trips, eligibility`). frontend: `supabase.ts` עם `db:{schema:'eligibility'}`, `.env` + env ב-Vercel (VITE_SUPABASE_URL/ANON_KEY → coe-hub). deploy CLI ל-https://eligibility-engine-sigma.vercel.app. **לקח**: `npx vercel env add` על Windows+Git Bash **לא קורא stdin** (שומר ערך ריק) — חובה לקרוא ל-binary ישירות (`.bin/vercel`). אימות חי ב-Playwright: REST ל-coe-hub `eligibility_checks?select=*,decisions(*)` → 200, 0 שגיאות קונסול, ה-ref הישן נעדר מה-bundle. נתונים היסטוריים ניתנים לייבוא מגיבוי בהמשך (המייל מבטיח הורדה גם אחרי הקפאה). |
| 15.09 | **fix(calculator): פריסת כותרת מובייל — ביטול גלילה אופקית**. במובייל שורת השלבים+הפעולות ב-`wizardProgress` נגללה אופקית (`overflow-x:auto`) — UX גרוע, במיוחד עם כפתור המחירון הארוך. תוקן ב-`@media(max-width:768px)`: `stepsRow` הפך ל-`column` (עיגולי שלבים ממורכזים ללא scroll בשורה 1, שורת פעולות בשורה 2), `overflow-x:visible`, `wizardActions` עם `flex-wrap`; class חדש `priceBtn` נותן לכפתור המחירון רוחב מלא (`flex:1 1 100%`) בעוד שמור+החישובים חולקים שורה. חל על שני המחשבונים (CSS משותף). אומת ב-Playwright 390px (data + AI). commit `ecf2c6d`. |
| 15.09 | **feat(knowledge-hub): הצ'אט עונה על כל סוגי השאלות — לא רק "מסעות"**. עד היום כל פנייה במרכז הידע הוכרחה להיות מסע רב-שלבי (Gemini החזיר `steps[]`, נשמר כ-journey ב-DB). המשתמש דרש שהצ'אט יענה גם על שאלות מידע ובקשות קובץ. **שינוי**: היועץ מסווג כל פנייה ל-`kind`: **`answer`** (שאלת מידע / בקשת קובץ / "מה זה…" / "כמה עולה…") → תשובה טקסטואלית inline **בלי שמירה ל-DB ובלי ניווט** + כפתורי משאבים; או **`journey`** (יציאה לתהליך) → כמו קודם. **הורדת קבצים מהצ'אט**: exporter משותף חדש [priceTableExport.ts](digitek-platform/src/lib/priceTableExport.ts) בונה את טבלת התעריפים off-screen ומוריד PDF/PNG בקליק (אותם נתונים מ-`data.ts`, בלי לפתוח את המחשבון). resource keys: `takam_price_table`, `aiml_price_table`. קבצים: `promptBuilder.ts` (סיווג + סעיף משאבים + דוגמאות answer), `types.ts` (`AdvisorKind`/`answer`/`resources`), `advisor.ts` (פרסור שתי הצורות + backward-compat), `useJourney.ts` (מחזיר `CreateResult` מובחן), `AnswerView.tsx`+CSS (חדש), `KnowledgeHubPage.tsx` (ענף answer + דוגמאות/כותרת חדשות), `stepActions.ts` (`urlForModule`). `npx tsc --noEmit` נקי. אומת מקומית: מסך חיפוש + נכונות נתוני ה-exporter (חי דרך dynamic import). ⚠️ זרימת ה-AI המלאה + לחיצת הורדה טרם אומתו — ה-preview חסום login ואין GEMINI key מקומי; דורש בדיקת המשתמש בפועל. commit `b036795`. |
| 15.09 | **feat(calculator): מחירון מלא לייצוא PDF/תמונה בשני המחשבונים**. נוסף כפתור "📋 מחירון מלא" בשורת הפעולות העליונה של שני המחשבונים (תכ"ם + AI/ML) — זמין מכל שלב בוויזרד. פותח מודאל עם טבלת מחירון מלאה בדף אחד + 2 כפתורי ייצוא: PDF (`html2pdf.js` הקיים) ותמונה PNG (`html-to-image` — dependency חדש). **תכ"ם**: 37 תפקידים מקובצים לפי 7 קטגוריות × דרגות א׳–ד׳ (₪/שעה, לפני מע"מ), A4 landscape. **AI/ML**: 16 תוצרים × 3 גדלים (קטן/בינוני/גדול, ₪/תוצר לפני מע"מ), A4 portrait. הכפתור זמין תמיד (גם ללא login / view-only) כי המחירון הוא קטלוג סטטי. רכיבים חדשים: [PriceListModal.tsx](digitek-platform/src/modules/takam-calculator/PriceListModal.tsx) + `.module.css`, [AimlPriceListModal.tsx](digitek-platform/src/modules/aiml-calculator/AimlPriceListModal.tsx) (משתף את ה-CSS של תכ"ם). `npx tsc --noEmit` נקי. אומת חי ב-Playwright: שני המודאלים, ייצוא PNG (כל 37 התפקידים בדף אחד), מובייל 390px, 0 שגיאות קונסול. |
| 15.09 | **fix(takam): תיקון תעריפים ושמות ב-`data.ts` מול נספח יג הרשמי**. הפעלת פיצ'ר המחירון חשפה אי-התאמות בין הנתונים שהוזנו ב-2026 לבין טבלת התעריפים הרשמית (נספח יג, תעריפי גג). המשתמש סיפק את המקור הרשמי; הצלבתי 37 שורות תא-תא (30 היו תקינות). **תעריפים** (הזחות/ערכים חסרים): 1.6 PMO `{a151,b202,c232}→{b151,c202,d232}`, 2.1 אחראי פרויקט `{b219,c284}→{b219,c248,d284}`, 6.1 אחראי מע׳ המידע — הסרת `d327` שגוי (שייך ל-6.2), 6.7 ארכיטקט פתרונות `{a275,b340,c358}→{b275,c340,d358}`. **שמות**: 1.2 "פיתוח"→"תפעול", 2.2 "מנחת"→"מנהל", 2.8→"איש ניתוח ויישום פלטפורמה"; יישור נוסף למקור: 2.4 "בודק תוכנה", 2.6 "אפיון חוויית וממשק משתמשים (ui-ux)", 2.9 "פיתוח DevOps", 6.1 "אחראי מע׳ המידע". `data.ts` מוגן (סעיף 7) — בוצע רק אחרי אישור מפורש + מול המקור בלבד, לא על סמך AI. `npx tsc --noEmit` נקי. commit `cc19b55`. |
| 15.09 | **feat/fix(calculator): מחירון מלא לייצוא + תיקון פגינציית PDF**. כפתור "📋 טבלת תעריפים מהוראת התכ"ם להורדה" בשני המחשבונים (תכ"ם + AI/ML), זמין מכל שלב, פותח מודאל עם טבלת מחירון מלאה בדף אחד + ייצוא PDF (`html2pdf.js`) ותמונה PNG (`html-to-image`, dep חדש). תכ"ם: 37 תפקידים × דרגות א׳–ד׳; AI/ML: 16 תוצרים × 3 גדלים; לפני מע"מ. רכיבים: [PriceListModal.tsx](digitek-platform/src/modules/takam-calculator/PriceListModal.tsx) + `.module.css`, [AimlPriceListModal.tsx](digitek-platform/src/modules/aiml-calculator/AimlPriceListModal.tsx). תיקון פגינציה: `pagebreak.avoid='tr'` + `break-inside:avoid` כדי ששורה לא תיחתך בין עמודים. אומת ב-Playwright (מודאלים, PNG, מובייל 390px, 0 שגיאות); ייצוא ה-PDF מפיל את דפדפן ה-headless — לא אומת חזותית. commits `e08e306`, `b61e5fa`, `0fe9053`. |
| 13.09 | **fix(knowledge-hub): שלב "ספקים" עדיין 0 — search מיותר מסנן יתר**. אחרי תיקון האשכול, האשכול היה נכון (ניהול מוצר, 7 ספקים) אבל השלב העביר גם `search="מנהל מוצר"` (מכלל 8 שמכריח מילות מפתח) → 0 תוצאות ("מנהל" ≠ "ניהול", וגם טקסט חופשי לא תואם שמות ספקים). המסך עצמו רמז: "7 ספקים באשכול הזה ללא חיפוש". תיקון ב-[advisor.ts](digitek-platform/src/modules/knowledge-hub/lib/advisor.ts) — כש-`normalizeSupplierClusters` מזהה אשכול ספקים תקין, מוחק `search` + `specialization` (האשכול הוא הסינון המדויק; השאר מיותר ומזיק). `npx tsc --noEmit` נקי. |
| 13.09 | **fix(knowledge-hub): שלב "ספקים" נשלח לאשכול שגוי**. משתמש ביקש "גיוס מנהל מוצר" והמסע שלח אותו לספקים באשכול "פיתוח וניתוח" במקום "ניהול מוצר". **שורש**: ב-[moduleCatalog.ts](digitek-platform/src/modules/knowledge-hub/lib/moduleCatalog.ts) `SUPPLIER_CLUSTERS` הכיל **רק 7 אשכולות טק** — כל 5 אשכולות הדיגיטל (כולל `product-management`) חסרו, אז ל-AI לא הייתה בכלל אפשרות לבחור נכון, והוא העתיק אשכול מדוגמה. **תיקון 3-שכבתי**: (1) `SUPPLIER_CLUSTERS` שוכתב לכל 12 האשכולות (slug=שם+domain), purpose עודכן ל-350 ספקים. (2) [advisor.ts](digitek-platform/src/modules/knowledge-hub/lib/advisor.ts) — `normalizeSupplierClusters` דטרמיניסטי: אם יש שלב brief/expertise עם cluster_id, אשכול הספקים + הכותרת + התיאור נגזרים ממנו דרך `EXPERTISE_TO_SUPPLIER_CLUSTER` (מקור אמת אחד). slug לא-תקין מה-AI נמחק (מציג הכל במקום יעד שגוי). (3) כלל 9 ב-[promptBuilder.ts](digitek-platform/src/modules/knowledge-hub/lib/promptBuilder.ts) — suppliers cluster חייב לתאום את אשכול הבריף. `npx tsc --noEmit` נקי. |
| 13.09 | **fix(knowledge-hub): מרכז ידע נכשל על שלב expertise — הרחבת CHECK constraint**. "מצא לי" במרכז הידע החזיר שגיאה `new row for relation "journey_steps" violates check constraint "journey_steps_module_key_check"`. הסיבה: מודול "אשכולות והתמחויות" חובר למרכז הידע (09.09) — ה-AI וה-catalog מכירים `module_key='expertise'` — אבל ה-CHECK ב-[037_knowledge_hub_journeys.sql](digitek-platform/supabase/migrations/037_knowledge_hub_journeys.sql) התיר רק `brief/takam/aiml/tenders/roved5/suppliers`. **תיקון**: [migration 042](digitek-platform/supabase/migrations/042_journey_steps_allow_expertise.sql) — drop+recreate ה-constraint עם `expertise`. הוחל ב-digitek-dev (חל מיד גם על preview — אותו DB). אומת ב-insert אמיתי של שלב expertise בטרנזקציה + rollback. |
| 13.09 | **feat(expertise): צפייה היברידית בספקים מתוך האשכולות + מיפוי אשכול→אשכול (סגירת הנקודה הפתוחה)**. הכפתור "צפה בספקים" ב-[Expertise.tsx](digitek-platform/src/modules/expertise/Expertise.tsx) ניווט ל-`/suppliers?search={שם ההתמחות}` והציג מודול ריק — כי הטקסונומיות שונות (אשכולות = נספח ב' 12 אשכולות; ספקים = נספח ד1/ד2, 5 דיגיטל + 7 טק). המשתמש חיפש התמחות דיגיטלית בעולם הטק → 0 תוצאות. **פתרון**: (1) [clusterMapping.ts](digitek-platform/src/modules/expertise/clusterMapping.ts) — מיפוי נקי 12→12 (`cluster_id` → `{domain, clusterSlug, clusterName}`) מבוסס slug (יציב). רק "עיצוב שירות"→"חווית משתמש" סמנטי; השאר התאמה מדויקת/כמעט. (2) [SuppliersDrawer.tsx](digitek-platform/src/modules/expertise/SuppliersDrawer.tsx) + `.module.css` — מגירה צדדית (מוקאפ A) שנשארת בתוך מודול האשכולות: שולפת מ-`v_winning_suppliers_flat` לפי domain+cluster_slug, chip נשמר של האשכול (ניתן להסרה + החזרה), חיפוש פנימי, כרטיסי ספקים עם מנו"ף/הסכם/גודל/תוקף, ESC/overlay לסגירה, responsive. (3) כפתור "גש למודול ספקים המלא" מנווט ל-`/suppliers?cluster={slug}` — ה-deep-link הקיים ב-Suppliers.tsx מחליף domain ומסנן אשכול אוטומטית (הפרמטרים עוברים מבחירה לבחירה). אומת חי ב-Playwright מול digitek-dev: אשכול "ניהול מוצר" הציג 7 ספקי דיגיטל אמיתיים, והמעבר למודול נחת על דיגיטל+ניהול מוצר מסונן. `npx tsc --noEmit` נקי. 3 מוקאפים ב-`public/mockups/expertise-suppliers-drawer/`. |
...
| 17.09 | **fix(roved5): צ'אט AI — תשובות נכונות + מהירות + כפתור "ראה עוד במסך הרגיל"**. המשתמש דיווח שתשובת הצ'אט ל"סריקת מסמכים" שגויה (ענה "אין שירות OCR" והחמיץ את **Kal-Docs**). שלושה באגים תוקנו ברצף עם אבחון חי: (1) הצ'אט סינן מראש ל-60 מועמדים לפי keyword+נרדפות — ההצפה דחפה את Kal-Docs מחוץ לרשימה שה-AI ראה → עכשיו נשלח **כל הקטלוג** (keyword-hits ראשונים). (2) הקטלוג המלא גרם ל-timeout (thinking tokens ~58ש') → נוסף `generationConfig.thinkingConfig.thinkingBudget=0` + `responseMimeType:'application/json'` → ~2ש'. (3) `JSON.parse` נכשל כשה-answer הכיל מירכאות לא-escaped ("אינו זמין" למרות 200) → נוסף `responseSchema` (structured output) שמחייב JSON תקין + helper `extractJsonObject` כ-fallback. **פיצ'ר חדש**: לכל תשובת צ'אט כפתור "🔍 ראה עוד אפשרויות במסך הרגיל" ([Roved5Chat.tsx](digitek-platform/src/modules/roved5/Roved5Chat.tsx) `onSearchInCatalog`) שמעביר את השאילתה לחיפוש הראשי (מנקה פילטרים, סוגר מגירה, גולל למעלה) — כך רואים את כל התוצאות ולא רק 5. אומת חי ב-Playwright: Kal-Docs חוזר, לחיצה על הכפתור → "סריקת מסמכים" בחיפוש → 59 תוצאות. commits `f872525`, `e0127cb`, `e892b6f`, `87da4b4`. |
| 17.09 | **feat(roved5): שדרוג עיצוב מלא לרמת המתחרה "נימבוס" (מוקאפ B)**. המשתמש הראה את [roved-5.ai.studio](https://roved-5.ai.studio/) ושאל למה הוא נראה טוב יותר. אבחון: אותו מידע, ליטוש + פיצ'רים. אחרי 3 מוקאפים ([public/mockups/roved5-redesign/](digitek-platform/public/mockups/roved5-redesign/)) המשתמש בחר **B (לוח בקרה)**. מומש end-to-end על אותה קומפוננטה (חל על `/layer5` ו-`/share/layer5`): **KPI hero** גרדיאנט ([Roved5Stats.tsx](digitek-platform/src/modules/roved5/Roved5Stats.tsx)) עם 5 אריחים קליקים (סה״כ/AWS/GCP/SaaS/Non) + "עודכן" (max approval_date); **טולבר** (חיפוש AI + מועדפים + ייצוא Excel/PDF + שיתוף/ניהול ב-!publicMode); **כרטיסים עשירים** (העתקת מק״ט, מועדף ★, השוואה ⇄, פס צבע פר ענן, הנחה מודגשת/BYOL, לינק מחירון, "פרטים מלאים ←"); **מועדפים** ב-localStorage (`roved5:favorites`) + פילטר; **השוואה** עד 4 (בר צף + [Roved5CompareModal.tsx](digitek-platform/src/modules/roved5/Roved5CompareModal.tsx) טבלה זו-מול-זו); **ייצוא** [roved5Export.ts](digitek-platform/src/modules/roved5/roved5Export.ts) (xlsx RTL + html2pdf); **סייען AI צף** ([Roved5Chat.tsx](digitek-platform/src/modules/roved5/Roved5Chat.tsx) + `roved5Chat()` ב-roved5AI.ts — צ'אט שיחתי דרך `/api/ai-advisor` עם מיני-כרטיסים clickable). `CloudFilter`/`TypeFilter` הועברו ל-types.ts. `npx tsc` נקי. אומת חי ב-Playwright (אנונימי): עיצוב desktop, צ'אט Gemini החזיר 5 שירותי גיבוי אמיתיים, השוואה (בר+מודל), מובייל 390px, 0 שגיאות, publicMode מסתיר שיתוף/ניהול. commit `62ca8ee` (+ מוקאפים `df9d9d2`). |
| 17.09 | **feat(roved5): כפתור שיתוף במודול — בחירה בין צפייה בלבד למערכת מלאה**. המשך לפיצ'ר הציבורי — במקום שהמשתמש יזכור את ה-URL, נוסף כפתור "🔗 שתף" בראש מודול רובד 5 (לכל משתמש מחובר, מוסתר ב-`publicMode`, לצד "ניהול מוצרים" של אדמין) שפותח דיאלוג עם 2 אפשרויות: **צפייה בלבד** → `${origin}/share/layer5` (ללא התחברות/ניהול) או **המערכת המלאה** → `${origin}/` (התחברות לפי הרשאות). הקישור הנבחר מוצג ב-input לקריאה + כפתורי 📋 העתק / 💬 וואטסאפ / 📧 מייל. רכיב חדש [Roved5ShareDialog.tsx](digitek-platform/src/modules/roved5/Roved5ShareDialog.tsx) — ללא DB, קישורים נגזרים מ-`location.origin`. `headerActions` flex-wrapper חדש ב-CSS. `npx tsc --noEmit` נקי. אומת ב-Playwright שהדף הציבורי עדיין נקי (הכפתור לא מופיע ב-publicMode); הכפתור הפנימי לא אומת אוטומטית (דורש התחברות). commit `2209eaf`. |
...
> **תאריך**: 17.09.2026
> **נושא**: שדרוג עיצוב רובד 5 לרמת המתחרה "נימבוס" (מוקאפ B) + קישור/כפתור שיתוף ציבורי

### מה קרה
- המשתמש הראה את המתחרה [roved-5.ai.studio](https://roved-5.ai.studio/) ("נימבוס"). אבחנתי: אותו מידע, הפער = ליטוש + פיצ'רים (KPI bar, מועדפים, השוואה, ייצוא, צ'אט AI, מיתוג).
- בנינו 3 מוקאפים; המשתמש בחר **B (לוח בקרה)** ומימשנו את כל 4 קבוצות הפיצ'רים, על הדף הפנימי והציבורי כאחד (`publicMode` מסתיר שיתוף/ניהול).
- קדם לכך: קישור ציבורי `/share/layer5` (בלי התחברות) + כפתור "🔗 שתף" במודול (צפייה בלבד / מערכת מלאה).
- **אימות חי (Playwright, אנונימי)**: הכל עובד — כולל צ'אט Gemini שהחזיר שירותי גיבוי אמיתיים, השוואה, מובייל 390px, 0 שגיאות.
- commits: `62ca8ee` (עיצוב), `df9d9d2` (מוקאפים), `2209eaf` (כפתור שיתוף), `9d260ac` (קישור ציבורי).

### נותר / אופציונלי
- [ ] QA ידני של המשתמש ב-preview (מועדפים נשמרים ברענון, ייצוא Excel/PDF יורד).
- [ ] merge ל-main לקישור ציבורי יציב על דומיין הפרודקשן.
- [ ] אופציונלי: view toggle גריד↔טבלה (לא נכלל ב-B; אפשר להוסיף בטולבר).

---

## (היסטוריית שיחה קודמת — קישור ציבורי רובד 5)
...
### מה קרה
- המשתמש סיפק דשבורד HTML ("מפת ההתמחויות — דיגיטק") וביקש לבנות ממנו מודול חדש בליבה בשם **"אשכולות והתמחויות"**, עם: התאמת UI/UX לליבה, 3 מוקאפים לפני, פונקציונליות טובה יותר מה-HTML, חיפוש חכם AI (איזה אשכול מתאים לבעיה שלי), והוספה לתוצאות במרכז הידע.
- **המקור**: 12 אשכולות, 63 התמחויות, לכל אחת תיאור + פעילויות (`acts`) + תוצרים (`outs`) + scope + תאריך. **תובנה**: 12 האשכולות זהים בדיוק (שם+סדר) ל-`BRIEF_CLUSTERS` (id 1-12) — אז השתמשתי ב-cluster_id 1-12 שמיישר את המודול עם הבריפים ומרכז הידע.
- **החלטות שאושרו**: אחסון ב-Supabase (digitek-dev), קישור מלא לספקים זוכים, AI מחזיר אשכול+התמחויות+הסבר וגם כפתור "בנה מסע".
- **מוקאפים**: סבב 1 (`public/mockups/clusters-specs/` — A מגזין / B מאסטר-דיטייל / C AI-first). המשתמש ביקש שילוב **B+C** עם B כמסך ראשי, וציין שהנראות של סבב 1 סטתה מצבעי ליבה. סבב 2 (`public/mockups/clusters-specs-v2/` — v1/v2/v3) תוקן לצבעי ליבה בלבד. **המשתמש בחר וריאציה 2** (נחיתת מפת-חום + AI → מאסטר-דיטייל).
- **מומש end-to-end**: migration 041 (2 טבלאות + RLS, seed 12+63 שנטען דרך extension `http` מהכתובת הציבורית), `src/modules/expertise/` (types/hook/AI/component/CSS), route `/expertise`, פריט Sidebar 🧩, אינטגרציה למרכז ידע (ModuleKey/catalog/stepActions + `?wish=` ב-KnowledgeHub). `npx tsc --noEmit` נקי. אומת ב-Playwright מקומי + curl ל-AI endpoint על ה-preview (Gemini תקין).

### ✅ נסגר (13.09.2026) — מבנה ה"ספקים" והקישור מהתמחות
- **הנקודה הפתוחה נפתרה**: המיפוי הוא **אשכול→אשכול** (החלטת המשתמש). נספח ב' (12 אשכולות) ממופה נקי ל-12 אשכולות הספקים (5 דיגיטל + 7 טק) ב-[clusterMapping.ts](digitek-platform/src/modules/expertise/clusterMapping.ts). המיפוי ברמת **אשכול** ולא ברמת התמחות (הטקסונומיות של ההתמחויות שונות בין נספח ב' לד1/ד2). הקישור החדש: "צפה בספקים" פותח **מגירה** בתוך מודול האשכולות (לא ניווט החוצה), ורק "גש למודול ספקים" מנווט ל-`/suppliers?cluster={slug}`. פרטים מלאים בשורת ההיסטוריה 13.09.
- **פתוח קל (לא קריטי)**: מיפוי #1 "עיצוב שירות"→"חווית משתמש" הוא סמנטי — אם המשתמש ירצה, אפשר לכוונן. אין מיפוי ברמת התמחות (מכוון).

### (היסטוריית שיחה קודמת — COE Recruit)
...
### המשך (באותו יום): מוקאפ A נבחר ומומש במלואו
- migration 040 + Edge Function v2 + טעינת 202 ספקי דיגיטל + frontend עם טוגל — פרטים מלאים בשורת ההיסטוריה feat(suppliers) של 07.08
- אומת חי ב-Playwright (כולל תפיסת באג תקרת 1000 שורות של PostgREST)

### עוד לא בוצע
- [ ] QA ידני של המשתמש ב-Vercel preview על `/suppliers`
- [ ] אופציונלי בעתיד: הצגת שדות ערבות/ביטוח/סל הצמדה של נספח ד1 (קיימים באקסל, לא נטענים ל-DB — בפועל כמעט תמיד "לא")

...
### ⚠️ פתוח לשיחה הבאה — קריטי
- **מבנה ה"ספקים"**: המשתמש העלה שיש לי אי-הבנה לגבי מה הם "ספקים" והקשר שלהם להתמחויות. הוחלט "נתקן אחרי כן". כרגע הקישור מכל התמחות → `/suppliers?search={שם ההתמחות}` הוא **provisional** (deep-link פשוט), והמספרים המזויפים ("N ספקים זכו") הוסרו כבר בשלב המוקאפים. **לפני חיבור אמיתי** — לברר עם המשתמש: האם טקסונומיית נספח ב' (12 אשכולות) זהה לנספח ד1/ד2 שבמודול הספקים, ואיך בדיוק ממפים התמחות→ספקים זוכים.
